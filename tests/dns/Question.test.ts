...
describe('question records', () => {
  testProp('parse', [fc_question_no_unicast], (originalQuestion) => {
    ...
    const parsedQuestion = parseQuestionRecord(
      generatedQuestion,
      generatedQuestion,
    );
  ...
  testProp(
    'parse single string name',
    [fc_question_no_unicast],
    (originalQuestion) => {
  ...
  testProp(
    ...
    (originalQuestion, randomDomain) => {
      ...
      expect(decodedQuestion.data[1].name).toEqual(
        `${randomDomain}.${originalQuestion.name}`,
      );
      expect(decodedQuestion.remainder.length).toEqual(0);
    },
  );
